    ...
    selfAjax("post", "/admin/orderList/search?" + pStr, filter, function(data) {
        if (data && data.orders.length > 0) {
            data.orders.forEach(function(order) {
                var $tr = $('<tr id=' + order._id + '><td>' + order.name + '</td><td>' +
                    order.sCount + '</td><td>' + order.schoolArea + '</td><td><div class="btn-group">' + getButtons() + '</div></td></tr>');
                $tr.find(".btn-group").data("obj", order);
...
$("#btnSave").on("click", function(e) {
    ...
    {
        var postURI = "/admin/order/add",
            postObj = {
            name: $.trim($('#name').val()),
            address: $.trim($('#address').val())
        };
        if (!isNew) {
            postURI = "/admin/order/edit";
            postObj.id = $('#id').val();
        }
...
$("#gridBody").on("click", "td .btnDelete", function(e) {
    ...
    $("#btnConfirmSave").off("click").on("click", function(e) {
        selfAjax("post", "/admin/order/delete", {
            id: entity._id
        }, function(data) {
            if (data.error) {
                showAlert(data.error);
