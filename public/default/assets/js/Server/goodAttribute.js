...
function search(p) {
    ...
    selfAjax("post", "/admin/goodAttributeList/search?" + pStr, filter, function(data) {
        if (data && data.goodAttributes.length > 0) {
            data.goodAttributes.forEach(function(goodAttribute) {
                var $tr = $('<tr id=' + goodAttribute._id + '><td>' + goodAttribute.name + '</td><td>' +
                    goodAttribute.sCount + '</td><td>' + goodAttribute.schoolArea + '</td><td><div class="btn-group">' + getButtons() + '</div></td></tr>');
                $tr.find(".btn-group").data("obj", goodAttribute);
