    ...
    var pageManager = {
        ...
        pageInitEvents: function () {
            ...
            $("#btnAdd").on("click", function (e) {
                ...
                $('#myModal #myModalLabel').text("新增套件");
                $('#myModal #id').val("");
                $("#myModal #name").val("");
                $("#myModal #suiteId").val("");
                $("#myModal #secret").val("");
            ...
            $("#myModal #btnSave").on("click", function (e) {
                ...
                if (validator.isValid()) {
                    var postURI = "/admin/sysSuit/add",
                        postObj = {
                            name: $.trim($("#myModal #name").val()),
                            suiteId: $.trim($("#myModal #suiteId").val()),
                            secret: $.trim($("#myModal #secret").val()),
                        };
                    if ($('#id').val()) {
                        postURI = "/admin/sysSuit/edit";
            ...
            $("#gridBody").on("click", "td .btnEdit", function (e) {
                ...
                $("#myModal #suiteId").val(entity.suiteId);
                $("#myModal #secret").val(entity.secret);
            ...
            $("#gridBody").on("click", "td .btnDelete", function (e) {
                ...
                $("#btnConfirmSave").off("click").on("click", function (e) {
                    selfAjax("post", "/admin/sysSuit/delete", {
                        id: entity._id
                    }, function (data) {
                        if (data.error) {
                            showAlert(data.error);
        ...
        pageSearch: function (p) {
            ...
            selfAjax("post", "/admin/sysSuitList/search?" + pStr, filter, function (data) {
                if (data && data.records.length > 0) {
                    var d = $(document.createDocumentFragment());
                    data.records.forEach(function (record) {
                        var $tr = $('<tr id=' + record._id + '><td>' + record.name + '</td><td>' +
                            record.suiteId + '</td><td><div class="btn-group">' + that.pageGetButtons() + '</div></td></tr>');
